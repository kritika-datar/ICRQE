...
from ICRQE.llm.build_llm_client import (build_llm_chat_client,
                                        build_llm_completion_client)
from ICRQE.storehouse.chromadb_utils import init_db, populate_db
from ICRQE.tools.tools_to_yaml import (save_tools_to_yaml, setup_and_load_yaml,
                                       setup_tools)
...
@click.command()
@click.option(
    "--config", "-c", default="config.yaml", help="Path to the config yaml file"
)
@click.option(
    "--model",
    "-m",
...
def main(config: str, model: str):
    ...
    collection, files = init_db(
        client, os.path.expanduser(config.repository), config.extensions
    )
    files = sorted(files)
    ...
    for filepath in files:
        summarize_file(
            filepath, os.path.expanduser(config.repository), llm, descriptions
        )
